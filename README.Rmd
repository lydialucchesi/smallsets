...
* Look at adding an occurrence rate tag ("this type of issue occurs 1 in every 100 rows of data") - maybe this would be part of the timeline analysis tools or a way to help with the sampling bias issue mentioned above
  * Could also adjust tile size or tile border width or tile transparency depending on occurrence rate
* Work on un-abstracted timeline
  * Overall create_timeline function - abstract argument and if `TRUE` then run abstract_timeline?
* ~~Read in captions as R markdown expressions so that you can format text (bold, italicize, etc.)~~
  * ~~Bold letter/word corresponding to stamp~~
* Figure out what to do if I merge two data sets - will need to sample and create two separate smallsets?
  * Need to adjust prep_smallset to accept a list of data sets then (not just one)
* Need to address some errors
  * You get an error if you pass "data" to data argument in prep_smallset - fix?
  * Weather conditions error... this one is going to be difficult.
* Figure out what to do with the tile colour fill when a data point has been changed twice in a between two timeline points

## 11 November 2020

The researcher view rough draft. Because it is the "researcher" view, the data in the smallset do not need to be hidden/abstracted). Need to find a good way to add a colour key. Captions should be more extensive - just threw down some random text that I made up. All of these data are made up just for testing purposes.

Now squares are all the same size across plots. Users can specify a font, add titles and subtitles, choose one of five stamp locations, and either abstract the data or not.

The code for the timeline above would look someting like this (and you'd fill out the caption template).
...
The only arguments I changed between the two views are the following in the create_timeline function: abstract, stampLoc, and the sizing for the circles and symbols.
...
The smallset package can now work with several smallsets simultaneously, so you can visualise merging datasets in a smallset timeline.
