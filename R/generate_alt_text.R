...
generate_alt_text <-
  function(title,
    ...
    if (("changed1" %in% row.names(snapshotList[[9]])) | (snapshotList[[6]] %in% snapshotList[[9]]$colValue)) {
      ...
      suppressWarnings(brew(file = "altTextTemplates/editColour.txt", output = "altText/intro_4.txt"))
    }
    ...
    if (("added1" %in% row.names(snapshotList[[9]])) | (snapshotList[[7]] %in% snapshotList[[9]]$colValue)) {
      ...
      suppressWarnings(brew(file = "altTextTemplates/addColour.txt", output = "altText/intro_5.txt"))
    }
    ...
    if (("deleted1" %in% row.names(snapshotList[[9]])) | (snapshotList[[8]] %in% snapshotList[[9]]$colValue)) {
      ...
      suppressWarnings(brew(file = "altTextTemplates/delColour.txt", output = "altText/intro_6.txt"))
    }
    ...
    for (i in 1:length(snapshotList[[1]])) {
      ...
      suppressWarnings(brew(file = "altTextTemplates/snapDim.txt", output = "altText/body_1.txt"))
      
      if (i == 1) {
        suppressWarnings(brew(file = "altTextTemplates/colNames.txt", output = "altText/body_2.txt"))
        
        if (length(altTextInfo$rowsDrop) != 0) {
          if (length(altTextInfo$rowsDrop) > 1) {
            suppressWarnings(
              brew(file = "altTextTemplates/delRows.txt", output = "altText/body_3.txt")
            )
            altTextInfo$rowsDrop <- NULL
          } else {
            suppressWarnings(brew(file = "altTextTemplates/delRow.txt", output = "altText/body_4.txt"))
            altTextInfo$rowsDrop <- NULL
          }
        } else {
          altTextInfo$rowsDrop <- NULL
        ...
        if (length(altTextInfo$colsDrop) != 0) {
          if (length(altTextInfo$colsDrop) > 1) {
            suppressWarnings(
              brew(file = "altTextTemplates/delColumns.txt", output = "altText/body_5.txt")
            )
            altTextInfo$colsDrop <- NULL
          } else {
            suppressWarnings(
              brew(file = "altTextTemplates/delColumn.txt", output = "altText/body_6.txt")
            )
            altTextInfo$colsDrop <- NULL
          }
        } else {
        ...
        if (nrow(altTextInfo$adjData) > 0) {
          if (nrow(altTextInfo$adjData) > 1) {
            suppressWarnings(
              brew(file = "altTextTemplates/editCells.txt", output = "altText/body_7.txt")
            )
            altTextInfo$adjData <- NULL
          } else {
            suppressWarnings(
              brew(file = "altTextTemplates/editCell.txt", output = "altText/body_8.txt")
            )
            altTextInfo$adjData <- NULL
          }
        } else {
        ...
        if (length(altTextInfo$rowsAdd) != 0) {
          if (length(altTextInfo$rowsAdd) > 1) {
            suppressWarnings(
              brew(file = "altTextTemplates/addRows.txt", output = "altText/body_9.txt")
            )
            altTextInfo$rowsAdd <- NULL
          } else {
            suppressWarnings(
              brew(file = "altTextTemplates/addRow.txt", output = "altText/body_10.txt")
            )
            altTextInfo$rowsAdd <- NULL
          }
        } else {
        ...
        if (length(altTextInfo$colsAdd) != 0) {
          if (length(altTextInfo$colsAdd) > 1) {
            suppressWarnings(
              brew(file = "altTextTemplates/addColumns.txt", output = "altText/body_11.txt")
            )
            altTextInfo$colsAdd <- NULL
          } else {
            suppressWarnings(
              brew(file = "altTextTemplates/addColumn.txt", output = "altText/body_12.txt")
            )
            altTextInfo$colsAdd <- NULL
          }
        } else {
