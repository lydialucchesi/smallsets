...
# check if the row and col add works
# would using subset(lprior, select=colnames(lcurrent)) mess up code later on
...
highlight_changes <- function(list) {
  ...
  for (p in 1:(length(list) - 1)) {
    ...
    if (length(rowsDrop) > 0) {
      lpriorAdj <- lprior[-as.integer(rowsDrop), ]
    } else {
      lpriorAdj <- lprior
    }
...
source("gen_data.R")
mylist <- prep_smallset(data = df, prepCode = "prep_data.R", size = 6, rowNums = c(1, 2, 5, 8, 9))
